...
function instructorName(inst: ClassInstance): string | null {
  return inst.instructor && typeof inst.instructor === 'object' ? inst.instructor.name : null
}

export default async function AllClasses({ initPageResult, params, searchParams }: AdminViewServerProps) {
  const { req, permissions, visibleEntities, locale } = initPageResult
  const { user, payload } = req
  if (!user || user.collection !== 'users') redirect('/admin/login')
  // This view is instructor-only; other staff use the collection list view for
  // "all classes" since they already read everything there.
  if (!user.roles?.includes('instructor')) redirect('/admin')

  // Every instance is returned: class-instances read access grants instructors
  // full read (overrideAccess: false + user still applies the collection rule).
  ...
  const renderSection = async (rows: ClassInstance[], emptyText: string) => {
    ...
    const cards = await Promise.all(
      rows.map(async (inst) => {
        ...
        const instructor = instructorName(inst)
        return (
  ...
  const upcomingNode = await renderSection(upcoming, 'No upcoming classes.')
  ...
  return (
    ...
    >
      <Gutter>
        <h1 style={{ marginBottom: 'var(--base)' }}>All Classes</h1>
